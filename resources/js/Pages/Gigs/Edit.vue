...
export default {
    layout: (_h, page) => h(AppLayout, { title: 'Edit gig' }, () => page),
};
</script>
...
const props = defineProps({
    // The gig being edited, shaped by GigController@edit.
...
// The gig's own currency leads the list so it's always selectable.
const currencies = [...new Set([props.gig.currency, 'USD', 'EUR', 'GBP', 'CAD', 'AUD'])];

// Y-m-d → local Date (no UTC midnight shift); H:i → a Date carrying that time.
...
const form = useForm({
    ...
    name: props.gig.name ?? '',
    venue_id: props.gig.venueId,
    date: parseDate(props.gig.date),
    load_in_time: parseTime(props.gig.loadInTime),
    soundcheck_time: parseTime(props.gig.soundcheckTime),
    doors_time: parseTime(props.gig.doorsTime),
    start_time: parseTime(props.gig.startTime),
    end_time: parseTime(props.gig.endTime),
    fee: props.gig.fee,
    currency: props.gig.currency,
    notes: props.gig.notes ?? '',
});
...
<template>
    ...
    <div class="mx-auto max-w-2xl">
        <div class="mb-6">
            <h2 class="font-display text-3xl font-bold tracking-tight">Edit gig</h2>
            <p class="mt-1 text-sm text-ink/60 dark:text-canvas/55">
                Update the details as the date firms up — change the status, lock in a venue,
                fill in the call sheet.
            </p>
        </div>
